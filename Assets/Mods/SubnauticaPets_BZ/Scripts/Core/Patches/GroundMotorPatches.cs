...
namespace DaftAppleGames.SubnauticaPets.Patches
{
    ...
    /// </summary>
    [HarmonyPatch(typeof(GroundMotor))] internal class GroundMotorPatches
    {
        ...
        {
            if (!go.transform.parent) return;
            if (go.transform.parent.TryGetComponent(out Pet pet)) __result = false;
        }
    }
}