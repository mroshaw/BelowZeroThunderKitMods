...
{
    [HarmonyPatch(typeof(MoveOnSurface))] internal class MoveOnSurfacePatch
    {
        /// <summary>
        ...
        private static bool Perform_Prefix(MoveOnSurface __instance, float time, float deltaTime)
        {
            ...
            var pet = __instance.gameObject.GetComponent<Pet>();
            if (!pet)
            ...
            __instance.timeNextTarget = time + __instance.updateTargetInterval +
                                        __instance.updateTargetRandomInterval * Random.value;
            // Enforce the "GoToInternal" behaviour
            __instance.walkBehaviour.GoToInternal(__instance.desiredPosition,
                (__instance.desiredPosition - creature.transform.position).normalized, __instance.moveVelocity);
            return false;
        }
    }
}