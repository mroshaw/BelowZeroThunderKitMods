...
namespace DaftAppleGames.SubnauticaPets.Utils
{
    ...
    internal class AlignToFloorOnStart : MonoBehaviour
    {
        ...
        {
            var isHit = Physics.Raycast(transform.position, Vector3.down, out var hit, 5.0f);
            if (isHit)
            {
            ...
            ModDebugLog.LogDebug(
                $"AlignToFloorOnStart: Raycast failed to hit while aligning object {gameObject.name} to floor.");
